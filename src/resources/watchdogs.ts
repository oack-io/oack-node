...
import type { CreateWatchdogParams, UpdateWatchdogParams, Watchdog } from "../types/status-pages.js";
...
export class Watchdogs {
	...
	async create(accountId: string, pageId: string, compId: string, params: CreateWatchdogParams): Promise<Watchdog> {
		return JSON.parse(
			await this.client.request(
				"POST",
				`/api/v1/accounts/${accountId}/status-pages/${pageId}/components/${compId}/watchdogs`,
				{ json: params },
			),
		) as Watchdog;
	}
	...
	async update(
		...
		compId: string,
		watchdogId: string,
		params: UpdateWatchdogParams,
	): Promise<Watchdog> {
		return JSON.parse(
			await this.client.request(
				"PUT",
				`/api/v1/accounts/${accountId}/status-pages/${pageId}/components/${compId}/watchdogs/${watchdogId}`,
				{ json: params },
			),
		) as Watchdog;
	}
