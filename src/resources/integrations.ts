...
export class Integrations {
	...
	async createPagerDuty(accountId: string, params: CreatePDIntegrationParams): Promise<PDIntegration> {
		return JSON.parse(await this.client.request("POST", `/api/v1/accounts/${accountId}/integrations/pagerduty`, { json: params })) as PDIntegration;
	}

	async getPagerDuty(accountId: string): Promise<PDIntegration> {
		return JSON.parse(await this.client.request("GET", `/api/v1/accounts/${accountId}/integrations/pagerduty`)) as PDIntegration;
	}

	async updatePagerDuty(accountId: string, params: UpdatePDIntegrationParams): Promise<PDIntegration> {
		return JSON.parse(await this.client.request("PUT", `/api/v1/accounts/${accountId}/integrations/pagerduty`, { json: params })) as PDIntegration;
	}
	...
	async createCloudflare(accountId: string, params: CreateCFIntegrationParams): Promise<CFIntegration> {
		return JSON.parse(await this.client.request("POST", `/api/v1/accounts/${accountId}/integrations/cloudflare-zone`, { json: params })) as CFIntegration;
	}

	async listCloudflare(accountId: string): Promise<CFIntegration[]> {
		return JSON.parse(await this.client.request("GET", `/api/v1/accounts/${accountId}/integrations/cloudflare-zone`)) as CFIntegration[];
	}

	async getCloudflare(accountId: string, integrationId: string): Promise<CFIntegration> {
		return JSON.parse(await this.client.request("GET", `/api/v1/accounts/${accountId}/integrations/cloudflare-zone/${integrationId}`)) as CFIntegration;
	}

	async updateCloudflare(accountId: string, integrationId: string, params: UpdateCFIntegrationParams): Promise<CFIntegration> {
		return JSON.parse(await this.client.request("PUT", `/api/v1/accounts/${accountId}/integrations/cloudflare-zone/${integrationId}`, { json: params })) as CFIntegration;
	}
