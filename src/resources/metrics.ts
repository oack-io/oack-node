...
export class Metrics {
	...
	async createChartEvent(teamId: string, params: CreateChartEventParams): Promise<ChartEvent> {
		return JSON.parse(
			await this.client.request("POST", `/api/v1/teams/${teamId}/chart-events`, { json: params }),
		) as ChartEvent;
	}
	...
	async updateChartEvent(teamId: string, eventId: string, params: UpdateChartEventParams): Promise<ChartEvent> {
		return JSON.parse(
			await this.client.request("PUT", `/api/v1/teams/${teamId}/chart-events/${eventId}`, { json: params }),
		) as ChartEvent;
	}
