...
import type { ExternalLink, CreateExternalLinkParams, UpdateExternalLinkParams, CreateExternalLinkResult } from "../types/external-links.js";
...
export class ExternalLinks {
	...
	async create(teamId: string, params: CreateExternalLinkParams): Promise<CreateExternalLinkResult> {
		return JSON.parse(await this.client.request("POST", `/api/v1/teams/${teamId}/external-links`, { json: params })) as CreateExternalLinkResult;
	}
	...
	async get(teamId: string, linkId: string): Promise<ExternalLink> {
		return JSON.parse(await this.client.request("GET", `/api/v1/teams/${teamId}/external-links/${linkId}`)) as ExternalLink;
	}

	async update(teamId: string, linkId: string, params: UpdateExternalLinkParams): Promise<ExternalLink> {
		return JSON.parse(await this.client.request("PUT", `/api/v1/teams/${teamId}/external-links/${linkId}`, { json: params })) as ExternalLink;
	}
	...
	async assign(teamId: string, monitorId: string, externalLinkId: string): Promise<void> {
		await this.client.request("POST", `/api/v1/teams/${teamId}/monitors/${monitorId}/external-links`, { json: { external_link_id: externalLinkId } });
	}
	...
	async listMonitorLinks(teamId: string, monitorId: string): Promise<ExternalLink[]> {
		return JSON.parse(await this.client.request("GET", `/api/v1/teams/${teamId}/monitors/${monitorId}/external-links`)) as ExternalLink[];
	}
}
