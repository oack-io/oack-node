...
import type { Comment, CommentEdit, CreateCommentParams } from "../types/comments.js";
...
export class Comments {
	...
	async create(teamId: string, monitorId: string, params: CreateCommentParams): Promise<Comment> {
		return JSON.parse(
			await this.client.request("POST", `/api/v1/teams/${teamId}/monitors/${monitorId}/comments`, { json: params }),
		) as Comment;
	}
	...
	async edit(teamId: string, monitorId: string, commentId: string, body: string): Promise<Comment> {
		return JSON.parse(
			await this.client.request("PUT", `/api/v1/teams/${teamId}/monitors/${monitorId}/comments/${commentId}`, {
				json: { body },
			}),
		) as Comment;
	}
	...
	async reply(teamId: string, monitorId: string, commentId: string, body: string): Promise<Comment> {
		return JSON.parse(
			await this.client.request("POST", `/api/v1/teams/${teamId}/monitors/${monitorId}/comments/${commentId}/replies`, {
				json: { body },
			}),
		) as Comment;
	}
	...
	async listReplies(teamId: string, monitorId: string, commentId: string): Promise<Comment[]> {
		return JSON.parse(
			await this.client.request("GET", `/api/v1/teams/${teamId}/monitors/${monitorId}/comments/${commentId}/replies`),
		) as Comment[];
	}
	...
	async listEdits(teamId: string, monitorId: string, commentId: string): Promise<CommentEdit[]> {
		return JSON.parse(
			await this.client.request("GET", `/api/v1/teams/${teamId}/monitors/${monitorId}/comments/${commentId}/edits`),
		) as CommentEdit[];
	}
