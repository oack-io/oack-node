...
export class Postmortems {
	...
	async create(
		accountId: string,
		incidentId: string,
		params: CreatePostmortemParams,
	): Promise<Postmortem> {
		return JSON.parse(
			await this.client.request(
				"POST",
				`/api/v1/accounts/${accountId}/incidents/${incidentId}/postmortem`,
				{ json: params },
			),
		) as Postmortem;
	}
	...
	async publish(accountId: string, incidentId: string): Promise<Postmortem> {
		return JSON.parse(
			await this.client.request(
				"POST",
				`/api/v1/accounts/${accountId}/incidents/${incidentId}/postmortem/publish`,
			),
		) as Postmortem;
	}
	...
	async generateShareToken(accountId: string, incidentId: string): Promise<string> {
		const resp = JSON.parse(
			await this.client.request(
				"POST",
				`/api/v1/accounts/${accountId}/incidents/${incidentId}/postmortem/share`,
			),
	...
	async createTemplate(
		accountId: string,
		params: CreatePostmortemTemplateParams,
	): Promise<PostmortemTemplate> {
		return JSON.parse(
			await this.client.request(
				"POST",
				`/api/v1/accounts/${accountId}/postmortem-templates`,
				{ json: params },
			),
	...
	async getTemplate(accountId: string, templateId: string): Promise<PostmortemTemplate> {
		return JSON.parse(
			await this.client.request(
				"GET",
				`/api/v1/accounts/${accountId}/postmortem-templates/${templateId}`,
			),
		) as PostmortemTemplate;
	}
